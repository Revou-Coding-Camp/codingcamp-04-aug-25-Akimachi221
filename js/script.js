...
function renderTasks(taskArray = tasks) {
    ...
    if (taskArray.length === 0) {
        taskList.innerHTML = `
            <tr>
                <td colspan="4" class="text-center py-2 text-gray-500 border-[#F8F8F8]">No tasks available</td>
            </tr>
        `;
        return;
    }
    ...
    taskArray.forEach((task, index) => {
        ...
        taskList.innerHTML += `
            <tr class="bg-[#86A6DF]">
                <td class=" border px-4 py-2 font-semibold text-[#F8F8F8]">${task.title}</td>

                <td class=" border px-4 py-2 text-[#F8F8F8]">${task.date}</td>
